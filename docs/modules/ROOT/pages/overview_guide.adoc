...
image:https://www.threatx.com/wp-content/uploads/2022/02/getting-started.svg[Using ThreatX]

image:files/getting-started.svg[Using ThreatX]
...
image:Architecture-high-level.pngArchitecture-high-level,width=2289,height=1214]
...
The Decision Engine classifies and scores the threats as follows:
...
The rule’s action can include the following:
...
The ThreatX blocking modes are designed to block malicious requests and deter suspicious entities from attacking your sites while allowing benign traffic and real users through. There are three different blocking modes available for each site: 
...
The Cloud Analytics is a single analytics engine that receives feeds from different detection techniques. The Cloud Analytics has two main components:
...
*NOTE*: You can import your own threat intelligence. The ThreatX platform can use deny lists from threat intelligence solutions by integrating with SOAR solutions or by scripting using the ThreatX API.
...
See the https://www.threatx.com/documentation/using-threatx/threatx-administrator-guide/[ThreatX Managed API and Application Protection Administrator Guide] for details about managing the platform.
...
If the ThreatX Risk-Based Blocking feature is enabled, an administrator or ThreatX SOC can configure the following settings: 
...
* Site groups. You can assign a site group to limit which users can access the site configuration and its associated data. 
...
For information about sensor deployment, see the link:#deployment[Deployment] section.
...
The https://www.threatx.com/documentation/using-threatx/threatx-analyst-guide/[ThreatX Managed API and Application Protection Analyst guide] focuses on those areas of the ThreatX platform that provide analytical data.
...
The ThreatX platform supports two types of Edge Caching:
...
ThreatX offers the following simple methods of deployment: 
...
* ThreatX Hybrid Deployment. Mix of the ThreatX cloud, public cloud, and Docker deployments deployed when a single deployment model is not feasible. ThreatX will work with the customer to map out the optimal configurations and support models. 

For more information, see the _https://www.threatx.com/documentation/deployment-guides/[ThreatX Sensor Deployment Guides]_.
...
*NOTE*: The ThreatX sensors were not designed to monitor site uptime. The ThreatX sensors only see and act on ingress HTTP(s) traffic. Due to the WAFs position in front of your inbound traffic, it is not afforded the same level of insight that a purpose-built monitoring solution would be able to provide. 
...
* link:#additional-features[Additional Features]
* link:#privacy[Privacy]
* link:#deployment[Deployment]
* link:#onboarding-checklist[Onboarding Checklist]
...
https://www.threatx.com/documentation/using-threatx/getting-started/[READ MORE]
...
https://www.threatx.com/documentation/using-threatx/threatx-administrator-guide/[READ MORE]
...
https://www.threatx.com/documentation/using-threatx/threatx-analyst-guide/[READ MORE]
